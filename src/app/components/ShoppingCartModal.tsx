 ...
 async function handleCheckoutClick(event:React.MouseEvent<HTMLButtonElement>){
  ...
  return (
    <Sheet
      ...
      onOpenChange={(isOpen) => {
        if (!isOpen) {
          handleCartClick(); // Close the cart only if it's open
        }
      }}
    >
                ...
                Object.values(cartDetails ?? {}).map((entry) => (
                  <li key={entry.id} className="py-6 flex rounded-md">
                    <div className="h-24 w-24 flex-shrink-0 overflow-hidden rounded-md border border-gray-200">
                      ...
                      <Image
                        src={entry.image ? entry.image : "/product1.png"} // Fallback image if no image found
                        alt={entry.name}
                        width={100}
                        height={100}
                    ...
                    <div className="flex flex-1 items-end justify-between text-sm">
                      ...
                      <div className="flex">
                        <button type="button" onClick={()=>removeItem(entry.id)} className="font-md text-primary hover:text-primary/80">Remove</button>
                      </div>
                    </div>
                    </div>
                  </li>
